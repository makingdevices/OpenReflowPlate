...
Making Devices - 2022 CDC FOR PIC18F14K50
...
char USB_Out_Buffer[32];		//USB OUT BUFFER
char SPI_response[8];
...
/** VECTOR REMAPPING ***********************************************/
    ...
    #define APP_FIRMWARE_VERSION_MINOR  20   //valid values 0-99
	#define APP_HARDWARE_VERSION_MAJOR  0x01   //valid values 0-255
    #define APP_HARDWARE_VERSION_MINOR  0x01   //valid values 0-99
    //--------------------------------------------------------------------------
...
{    
	T0CONbits.T08BIT = 0;  //150ms at 48Mhz. //TconvMAX31855 = 100ms
	T0CONbits.T0CS = 0;
	T0CONbits.PSA = 0;
	T0CONbits.T0PS2 = 1;
	T0CONbits.T0PS1 = 0;
...
{   
    ...
    if(USBUSARTIsTxTrfReady())
    {
		...
		if(numBytesRead != 0)
		{
			USB_In_Buffer[0] = 0x3F;
			USB_In_Buffer[1] = SPI_lastmessage[0];
			USB_In_Buffer[2] = SPI_lastmessage[1];
			USB_In_Buffer[3] = SPI_lastmessage[2];
			USB_In_Buffer[4] = SPI_lastmessage[3];
			USB_In_Buffer[5] = SPI_lastmessage[4];
			USB_In_Buffer[6] = SPI_lastmessage[5];
			USB_In_Buffer[7] = SPI_lastmessage[6];
			USB_In_Buffer[8] = SPI_lastmessage[7];
			USB_In_Buffer[9] = AppVersion[0];
			USB_In_Buffer[10] = AppVersion[1];
			USB_In_Buffer[11] = APP_HARDWARE_VERSION_MAJOR;
			USB_In_Buffer[12] = APP_HARDWARE_VERSION_MINOR;
			USB_In_Buffer[13] = 0x3F;
			putUSBUSART(USB_In_Buffer,14);
			out_status = USB_Out_Buffer[0];
		}
	}
